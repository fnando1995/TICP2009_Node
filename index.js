...
function solveOption1(userId,init){
  if (init=='help'){
    ...
    //-crypto- --btc
    request('https://api.coingecko.com/api/v3/simple/price?ids='+dict_coins[init].id+'&vs_currencies=usd',{ json: true }, (err, res, body)=> {
    if (err) { return console.log(err); }
    value =body[dict_coins[init].id]['usd']
...
// creating list of coins to interchange easily
